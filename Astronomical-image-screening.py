...
rec = []
#输入图像的路径
path = r'/home/awen/Awen/baddate/*.fit*'
#产生的数据保存
#str1="/home/awen/Awen/printdate.txt"
#file1=open(str1,'w+')  

y, x = svm_read_problem('/home/awen/Awen/train0.txt')
prob  = svm_problem(y, x)
#c与g的值有ezsy.py对train训练所得
param = svm_parameter('-t 0 -c 8192 -g 0.0078125 -q') #线性核 cost参数(1)   是否估算正确概率(0)
...
#按读入顺序排列
	...
	if length>0:
		...
		for k in range(0,length):
			...
			yt=[1]
			xt =[{1: float("%0.1f"%snrmy), 2: float("%0.1f"%np.max(ROAD)), 3: float("%0.1f"%wlk), 4: float("%0.1f"%(wenli0[0]*0.1)), 5: wenli0[1], 6: wenli0[2], 7: wenli0[3]*10}]
			print xt
			p_label, p_acc, p_val = svm_predict(yt,xt, model)
			
			if p_acc[0]<1:
				shutil.move("/home/awen/Awen/baddate/"+os.path.basename(rec[k]),"/home/awen/Awen/bad/"+os.path.basename(rec[k]))
			else:
				shutil.move("/home/awen/Awen/baddate/"+os.path.basename(rec[k]),"/home/awen/Awen/god/"+os.path.basename(rec[k]))
