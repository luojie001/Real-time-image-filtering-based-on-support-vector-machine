...
for k in range(0,10):
	...
	f=hh[0].data
	b=np.random.normal(120,50,(1024,1024))
	f=f+b
	f=f*256.0/np.max(f)
	Imgw=len(f) 
	Imgl=len(f[0])
	...
	#print k,rec[k]
	wenli='1:'+str(float("%0.1f"%snrmy))+' 2:'+str(float("%0.1f"%np.max(ROAD)))+' 3:'+str(float("%0.1f"%wlk))+' 4:'+str(wenli0[0]*0.1)+' 5:'+str(wenli0[1])+' 6:'+str(wenli0[2])+' 7:'+str(wenli0[3])
	print k,wenli
